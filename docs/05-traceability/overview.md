...
| **Nguồn sync** | DOC-03 · memory · DOC-08–12 Chốt · DOC-14/15 **Chốt** · DOC-16/17 **Chốt** · `hrm-backend` IAM EF + **Lark IdP** (DEC-DLV-010) |
...
| **Code slice (execute)** | IAM `hrm-backend` ◐ — Jarvis 5-layer · EF IAM + migrate PostgreSQL · roles SoT DB · `GET /v1/iam/me` · **IdP Lark** (issuer URL nợ IT) |
...
| employee-profile | BA | ✓ | ✓ | ✓ | ✓ | ✓ | — | TC chưa chạy |
| lifecycle | BA | ✓ | ✓ | ✓ | ✓ | ✓ | — | Git/CRM N+3; job không DR |
| identity | BA | ✓ | ✓ | ✓ | ✓ | ✓ | — | ADR-007 OIDC · **IdP Lark** (DEC-DLV-010); code ◐ EF+migrate; TC chưa chạy |
...
| M4c | First code slice | IAM `hrm-backend` Host + Application + EF IAM | 2026-08-28 | **done** ◐ (DB roles SoT; chưa Lark JWKS) |
...
| Slice IAM `hrm-backend` (Jarvis + Application + EF) | DEV | identity | 2026-08-28 | **done** ◐ (migrate OK; roles SoT PG; chưa JWT Lark) |
